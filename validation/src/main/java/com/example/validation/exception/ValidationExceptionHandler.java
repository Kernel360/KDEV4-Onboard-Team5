...
public class ValidationExceptionHandler {
    ...
    public ResponseEntity<Api> validationException(MethodArgumentNotValidException e) {
        ...
        var errorMessageList = e.getFieldErrors().stream().map(it -> {
            var format = "%s : { %s } 은 %s";
        ...
        var errorResponse = Api.builder().resultCode(String.valueOf(HttpStatus.BAD_REQUEST.value())).resultMessage(HttpStatus.BAD_REQUEST.name()).error(error).build();
